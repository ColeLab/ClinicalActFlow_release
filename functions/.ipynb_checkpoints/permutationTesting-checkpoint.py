...
##Modified by LJH April 7 2020 update to python 3
    #- note that multiprocessing has been removed because it was not compatible with python 3. Would be a relatively quick fix to implement it
...
def maxT(diff_arr, nullmean=0, alpha=.05, tail=0, permutations=1000, nproc=1, pvals=False):
    """
    ...
    for i in range(permutations):
        ...
        _maxT = _maxTpermutation(diff_arr,nullmean,tail,seed)
        result.append(_maxT)
        
        # this multiprocessing code has been commented out
...
def maxR(data_arr, behav_arr, alpha=.05, tail=0, permutations=1000, nproc=1, pvals=False):
    ...
    for i in range(permutations):
        seed = np.random.randint(0,100000,1)[0]
        result.append(_maxRpermutation(data_normed,behav_normed,tail,seed))
